...
public class Main extends Frame implements WindowListener, MineSweeperGUI {
    ...
    public Main(int n,int bom) {
        super("MineSweeper");
        ms = new MineSweeper(n, n, bom);  // 地雷が10個ある9×9の盤面
        init();
    }
	...
	int n;
	while(true){
	    System.out.print("マスの数:");
	    n=new java.util.Scanner(System.in).nextInt();
	    if(n>1){
		break;
	    }
	    System.out.println("不適切です");
	}
    
    int bom;
    while(true){
	System.out.print("地雷の数:");
	bom=new java.util.Scanner(System.in).nextInt();
	if(n*n>bom){
	    break;
	}
	System.out.println("不適切です");
